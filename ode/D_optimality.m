...
for rep=1:trial
    ...
    while(1)
        ...
        %% ACDS      
        [chosen_index] = D_optimal(Theta_true,Theta_predict,chosen_index,n_s,sigma,tol,tspan');
        time = chosen_index/100;
        x = xs(chosen_index,:);
        Xi_last = Xi;
...
%% output
dlmwrite('D_optimal2.txt',mean(error_gamma));
dlmwrite('D_optimal2.txt',std(error_gamma),'-append');
dlmwrite('D_optimal2.txt',mean(error_l2),'-append');
dlmwrite('D_optimal2.txt',std(error_l2),'-append');
